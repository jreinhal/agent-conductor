...
Debate flow: `BounceController` → `BounceOrchestrator` (event-driven state machine with states: idle → running → paused/waiting_user/consensus/max_rounds/judging/complete/error) → `POST /api/bounce` per participant per round → consensus analysis via TF-IDF cosine similarity + optional embeddings → judge synthesis. Supports mid-debate specialist tag-in (`PARTICIPANT_TAGGED_IN` event), participant pruning, and structured JSON responses (Zod-validated `StructuredBounceResponseSchema`).
...
- **`app/`** — Single-page app at `/`. API routes: `/api/chat` (streaming chat), `/api/bounce` (debate rounds), `/api/embed` (OpenAI text-embedding-3-small with 503 fallback), `/api/provider-status` (health check), `/api/decision-trace` (auto-routing details), `/api/model-dialogue`, `/api/protocol-board` (protocol session interface), `/api/session-insights`.
- **`components/`** — `SmartInput` (command input with `@model`, `$persona`, `#workflow`, `/command` prefixes + PII detection), `ChatPanel` (per-model streaming chat), `BounceController`/`BouncePanel` (debate orchestration UI with mid-debate specialist tag-in), `DebateReplay` (playback of completed debates with speed control), `SessionHistory` (slide-out panel with export + replay), `FirstRunWizard` (onboarding + provider status), `Canvas` (freeform draggable layout with physics), `ResizablePanels` (grid layout).
- **`lib/`** — Core logic. `store.ts` (Zustand with localStorage persistence), `ai.ts` (provider init), `models.ts` (10 static models across 4 providers + `auto-router`), `model-registry.ts` (dynamic fetcher with 5-min TTL cache for OpenAI/Anthropic/Google/xAI/Ollama), `personas.ts` (8 personas including repo-trained variants and debate moderator), `workflows.ts` (5 persona chains), `bounce-orchestrator.ts` (debate state machine with observer pattern, structured responses, specialist tag-in, convergence exit), `bounce-types.ts` (Zod `StructuredBounceResponseSchema`, full event/action unions, serialization helpers), `bounce-prompts.ts` (cross-pollination Proposal Board), `consensus-analyzer.ts` (TF-IDF cosine similarity + optional embedding-based similarity, near-tie runner-up detection), `export-utils.ts` (JSON/Markdown export with browser download), `guardrails.ts` (PII detection/redaction), `db.ts` (localStorage in browser, SQLite in Electron), `decision-router.ts` (deterministic auto-routing by prompt scoring), `simple-circuit-breaker.ts` (closed/open/half_open with configurable failure threshold and cooldown).
...
Zustand store in `lib/store.ts` with `persist` middleware. **Persisted** to localStorage via `partialize`: `sessions` (with messages), `sharedContext`, `sharedKnowledge`, `workflow.customWorkflows`, `debate.bounceHistory`, `debate.autoBounceEnabled`, `debate.minModelsForAutoBounce`. **In-memory only**: bounce state (current debate), selectedParticipants, bounceConfig, all UI state, activeWorkflow. Note: `sessionMessages` in `page.tsx` is local React state — not yet wired to the persisted store (known P0 gap). Electron uses SQLite via `lib/db.ts` as an alternative persistence backend.
